...
def compute_diff_map(next_flow, prev_flow, prev_frame, cur_frame, prev_frame_styled,occlusion_mask_blur, occlusion_mask_flow_multiplier,occlusion_mask_difo_multiplier, occlusion_mask_difs_multiplier):
  h, w = cur_frame.shape[:2]
  fl_w, fl_h = next_flow.shape[:2]

  prev_frame_styled = np.array(prev_frame_styled)[...,:3]
  prev_frame_styled = skimage.exposure.match_histograms(prev_frame_styled, cur_frame, channel_axis=None)
  prev_frame_styled = np.clip(prev_frame_styled, 0, 255).astype(np.uint8)
  ...
  alpha_mask = np.maximum.reduce([diff_mask_flow * occlusion_mask_flow_multiplier * 10, \
                                  diff_mask_org * occlusion_mask_difo_multiplier, \
                                  diff_mask_stl * occlusion_mask_difs_multiplier]) #
  ...
  if occlusion_mask_blur > 0:
    ...
    alpha_mask = cv2.GaussianBlur(alpha_mask, (blur_filter_size, blur_filter_size) , occlusion_mask_blur, cv2.BORDER_REFLECT)
